...
buttonpress(XEvent *e)
{
    ...
    // systray code (modified with original)
    else if (ev->x > selmon->ww - (int)TEXTW(stext) + lrpad - 2 - getsystraywidth())
    // systray code (seems like missing original parts)
...
drawbar(Monitor *m)
{
	...
	x = 0;
	//for (i = 0; i < LENGTH(tags); i++) {
	for (i = 0; i < LENGTH(tags) - 1; i++) { // hide the tag assigned to XK_0 or the 10th (9)
		w = TEXTW(tags[i]);
...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		resize(c, m->wx + m->gappoh, m->wy + m->gappov, m->ww - 2 * m->gappoh - 2 * c->bw, m->wh - 2 * m->gappov - 2 * c->bw, 0);
}
